/**
 * Learning Curve — Command Palette Component (TypeScript)
 * Encapsulated, state-driven search component with full keyboard accessibility.
 */
...
declare global {
  interface Window {
    ...
    CommandPaletteComponent?: typeof CommandPaletteComponent;
  }
}

export class CommandPaletteComponent {
  private catalog: CommandPaletteItem[];
  ...
  public mount(): this {
    ...
    const existing = document.getElementById("mintCmdOverlay");
    if (existing) {
      this.overlayEl = existing;
      this.inputEl = document.getElementById("mintCmdInput") as HTMLInputElement | null;
  ...
  private bindTriggers(): void {
    ...
    document.querySelectorAll(".mint-search-trigger, [data-open-cmd]").forEach(el => {
      el.removeEventListener("click", this.handleTriggerClick as EventListener);
      el.addEventListener("click", this.handleTriggerClick as EventListener);
    });
  }
  ...
  private onInput(): void {
    this.selectedIndex = 0;
    this.renderResults(this.inputEl ? this.inputEl.value.trim() : "");
  }
  ...
  public renderResults(query: string): void {
    if (!this.resultsEl) return;
...
// Built-in Default Catalog Dataset
export const defaultPaletteCatalog: CommandPaletteItem[] = [
...
export const cmdPaletteInstance = new CommandPaletteComponent({
